...
SMTP_PORT = 587
GMAIL_USERNAME = 'raspisecuriot@gmail.com'
GMAIL_PASSWORD = 'wognjpechintopkp'


def sendmail():
    # Define server and credentials in your cas

    sendTo = 'voanhtruong.utc@gmail.com'
    emailSubject = "Hello World"
    emailContent = ('Chào bạn, '
                    'Tôi muốn thông báo rằng tôi đã nhìn thấy một người lạ gần ngôi nhà của bạn.'
                    ' Xin lưu ý và đảm bảo an ninh cho gia đình và tài sản của bạn. '
                    'Trân trọng, '
                    'Raspi Smart Camera')
    ...
    # Attach the email content. Specify "plain" for text emails, or "html" for HTML emails
    ...
    # Send Email & Exit
    session.send_message(msg)  # send_message handles encoding issues gracefully
    session.quit()


# Sends an email to the "sendTo" address with the specified "emailSubject" as the subject and "emailContent" as the
# email content.
...
class Face_Recognizer:
    ...
    def attendance(self, name, img_rd):
        # Convert the captured image to binary
        success, encoded_image = cv2.imencode('.jpg', img_rd)
        if not success:
            print("Error converting image to binary format")
            return

        image_binary = encoded_image.tobytes()
        # Prepare time and date information
                    ...
                    # 6.2.2 / Face cnt increase: 0->1, 0->2, ..., 1->2, ...
                        ...
                        for k in range(len(faces)):
                            ...
                            if min(self.current_frame_face_X_e_distance_list) < 0.4:
                                ...
                                logging.debug(
                                    f"Face recognition result: {self.face_name_known_list[similar_person_num]}")

                                # Insert attendance record
                                nam = self.face_name_known_list[similar_person_num]

                                self.attendance(nam, img_rd)
                            else:
                                if self.last_email_time and time.time() - self.last_email_time < 60:
                                    print("Igone")
                                else:
                                    self.current_frame_face_name_list[k] = "Unknown person"
                                    nam = "Unknown person"
                                    self.attendance(nam, img_rd)
                                    sendmail()
